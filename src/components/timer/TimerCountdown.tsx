...
const TimerCountdown = ({
  ...
  return (
    <div
      className={cn(
        "grid items-center justify-center gap-4 dark:text-gray-200",
        timeLeft.days > 0 ? "grid-cols-4" : "grid-cols-3",
        variant === "large" && timeLeft.days > 0 ? "gap-6" : "gap-4",
        variant === "large" && timeLeft.days < 0 ? "gap-4" : "gap-4",
      )}
    >
      {timeLeft.days > 0 && (
        <div className="text-center">
          <div
            className={cn(
              "text-3xl font-bold",
              variant === "large" && "text-[84px] leading-tight",
            )}
          >
            {timeLeft.days}
          </div>
